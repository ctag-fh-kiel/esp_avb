...
typedef struct {
  ...
  uint8_t descriptor_index[2]; // descriptor index
  unique_id_t
      gptp_grandmaster_id; // gPTP grandmaster identity for this interface
  uint8_t
      propagation_delay[4]; // propagation delay in nanoseconds from ptp pdelay
  uint8_t gptp_domain_number;  // gPTP domain number of the GM on this interface
...
typedef struct {
   ...
   * Measures how far the talker's media clock (carried in stream AVTP
   * timestamps or CRF payload timestamps) has drifted from our local gPTP clock.
